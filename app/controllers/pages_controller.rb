class PagesController < ApplicationController
  ...
  if params[:search].blank?
    ...
    @parameter = params[:search].downcase
    @results = Certificados.all.where("lower(tratamiento) LIKE :search OR lower(id_droga) LIKE :search OR lower(id_vectores) LIKE :search OR lower(fecha_vencimientos) LIKE :search OR lower(id_cliente) LIKE :search", search: "%#{@parameter}%")
  end
  end
end
